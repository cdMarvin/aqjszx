...
class Common extends DbMysql {
     ...
     * +----------------------------------------------------------
     */
    function get_newshow_list($type = 'pc') {
        if ($type) {
            $where = " WHERE type = '$type'";
        }
        
        $sql = "SELECT * FROM " . $this->table('newshow') . $where . " ORDER BY sort ASC, id ASC";
        $query = $this->query($sql);
        while ($row = $this->fetch_array($query)) {
            $image = explode('.', basename($row['show_img']));
            $thumb = $GLOBALS['images_dir'] . $GLOBALS['thumb_dir'] . $image['0'] . "_thumb." . $image['1'];
     ...
     * +----------------------------------------------------------
     */
    function get_friendlink_list($cid=1, $type = 'pc') {
		$arr = array(
			'1' => '上级单位',	
			'2' => '成员单位',	
			'3' => '相关链接',	
        ...
        $sql = "SELECT * FROM " . $this->table('friendlink'). ' WHERE ' . $where . " ORDER BY sort ASC, id ASC";
        $query = $this->query($sql);
        while ($row = $this->fetch_array($query)) {
            $show_list[] = array (
                    "id" => $row['id'],
	...
	function get_all_article(){
		$sql = "SELECT * FROM " . $this->table('article_category');
        $query = $this->query($sql);
        while ($row = $this->fetch_array($query)) {
            $data[] = $row;
        }
