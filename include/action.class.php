...
class Action extends Common {
	...
	function get_url($cat_id, $str){
		$sql = "SELECT cat_id,parent_id, cat_name FROM " . $this->table('article_category') . " WHERE cat_id = '$cat_id'";
		$query = $this->query($sql);
		$one = array();
		while ($row = $this->fetch_array($query)) {
			$one = $row;
		}
		$str = '<a href="article_category.php?id='.$one['cat_id'].'">'.$one['cat_name'].'</a>&nbsp;&nbsp;'.(empty($str) ? '' : '>&nbsp;&nbsp;'.$str);

		if($one['parent_id']!=0){
			$str = $this->get_url($one['parent_id'], $str);
		}else{
			$str = $str;
		}
		return $str;
