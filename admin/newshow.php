...
$smarty->assign('cur', 'newshow');
$smarty->assign('ur_here', $_LANG['newshow']);
$smarty->assign('show_list', $dou->get_newshow_list());
...
elseif ($rec == 'insert') {
    ...
    $sql = "INSERT INTO " . $dou->table('newshow') . " (id, show_name, show_link, show_img, type, sort)" .
             " VALUES (NULL, '$_POST[show_name]', '$_POST[show_link]', '$file', 'pc', '$_POST[sort]')";
    $dou->query($sql);
    
    $dou->create_admin_log($_LANG['newshow_add'] . ': ' . $_POST[show_name]);
    $dou->dou_msg($_LANG['newshow_add_succes'], 'newshow.php');
} 
...
elseif ($rec == 'edit') {
    ...
    $query = $dou->select($dou->table('newshow'), '*', '`id` = \'' . $id . '\'');
...
elseif ($rec == 'update') {
    if (empty($_POST['show_name']))
        $dou->dou_msg($_LANG['newshow_name'] . $_LANG['is_empty']);
    ...
    $sql = "update " . $dou->table('newshow') . " SET show_name='$_POST[show_name]'" . $up_file .
             " ,show_link = '$_POST[show_link]', sort = '$_POST[sort]' WHERE id = '$_POST[id]'";
    $dou->query($sql);
    
    $dou->create_admin_log($_LANG['newshow_edit'] . ': ' . $_POST[show_name]);
    
    $dou->dou_msg($_LANG['newshow_edit_succes'], 'newshow.php');
} 
...
elseif ($rec == 'del') {
    // 验证并获取合法的ID
    $id = $check->is_number($_REQUEST['id']) ? $_REQUEST['id'] : $dou->dou_msg($_LANG['illegal'], 'newshow.php');
    
    $show_name = $dou->get_one("SELECT show_name FROM " . $dou->table('newshow') . " WHERE id = '$id'");
    
    if (isset($_POST['confirm']) ? $_POST['confirm'] : '') {
        // 删除相应商品图片
        $show_img = $dou->get_one("SELECT show_img FROM " . $dou->table('newshow') . " WHERE id = '$id'");
        ...
        $dou->create_admin_log($_LANG['newshow_del'] . ': ' . $show_name);
        $dou->delete($dou->table('newshow'), "id = $id", 'newshow.php');
    } else {
        $_LANG['del_check'] = preg_replace('/d%/Ums', $show_name, $_LANG['del_check']);
        $dou->dou_msg($_LANG['del_check'], 'newshow.php', '', '30', "newshow.php?rec=del&id=$id");
    }
}
