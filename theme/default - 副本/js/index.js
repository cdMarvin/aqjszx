...
var scrollElem1 = document.getElementById("ceng3");
var marqueesHeight1 = scrollElem1.style.height;
scrollElem1.onmouseover = new Function('stopscroll1 = true');
scrollElem1.onmouseout  = new Function('stopscroll1 = false');
var preTop1 = 0;
var currentTop1 = 0;
var stoptime1 = 0;
var leftElem1 = document.getElementById("ceng4"); 
scrollElem1.appendChild(leftElem.cloneNode(true));
init_srolltext3();

init_srolltext4();
function init_srolltext3(){
	scrollElem1.scrollTop = 0;
	setInterval('scrollUp2()', 165);//确定滚动速度的, 数值越小, 速度越快
}
function init_srolltext4(){
	scrollElem1.scrollTop = 0;
	setInterval('scrollUp2()', 165);//确定滚动速度的, 数值越小, 速度越快
}
function scrollUp2(){
	if(stopscroll1) return;
	currentTop1 += 2; //设为1, 可以实现间歇式的滚动; 设为2, 则是连续滚动
	if(currentTop1 == 19) {
		stoptime1 += 1;
		currentTop1 -= 1;
		if(stoptime1 == 180) {
